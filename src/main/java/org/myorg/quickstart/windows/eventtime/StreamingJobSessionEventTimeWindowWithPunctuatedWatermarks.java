...
/**
 * See explanation image of this program in <b>docs/windows-session-event-time-window-with-punctuated-watermarks.png</b>.
 *
 * <b>Session windows useful documentation:</b>
 * <li>https://ci.apache.org/projects/flink/flink-docs-stable/dev/stream/operators/windows.html#session-windows</li>
 * <br>
 ...
 */
public class StreamingJobSessionEventTimeWindowWithPunctuatedWatermarks {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Event> dataStream = env.fromElements(
                // a
                Event.builder().key("a").value(1).timestamp(5L).build(),   // window 1
                Event.builder().key("a").value(2).timestamp(6L).build(),   // window 1
                Event.builder().key("a").value(3).timestamp(7L).build(),   // window 1
                Event.builder().key("a").value(4).timestamp(12L).build(),  // window 1
                Event.builder().key("a").value(5).timestamp(17L).build(),  // window 1
                Event.builder().key("a").value(6).timestamp(23L).build(),  // window 2
                Event.builder().key("a").value(7).timestamp(24L).build(),  // window 2
                Event.builder().key("a").value(8).timestamp(25L).build(),  // window 2
                Event.builder().key("a").value(9).timestamp(31L).build(),  // window 2
                Event.builder().key("a").value(10).timestamp(30L).build(), // window 2
                Event.builder().key("a").value(11).timestamp(37L).build(), // window 3
                Event.builder().key("a").value(12).timestamp(38L).build()  // window 3
        );
        //endregion
        ...
        //region Group events with the same key into session windows (with static session gap of 5 milliseconds)
        ...
        //
        // A session window assigner can be configured with either a static session gap or
        // with a session gap extractor function which defines how long the period of inactivity is.
        //
        ...
        WindowedStream<Event, String, TimeWindow> windowedDataStream = dataStreamWithTimestampsAndWatermarks
                ...
                // Session event time window with static session gap of 5 milliseconds
        ...
        env.execute("Flink Streaming Job (SessionEventTimeWindowWithPunctuatedWatermarks example)");
        //endregion
    }
